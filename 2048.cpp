...
struct expectimax {
	...
	template<utils::estimator estim = utils::estimate, bool fixed = true>
...
handle specialize(utils::options& opts, const std::string& type) {
	...
	typedef utils::specialization<
			...
			index6t<0x3,0x4,0x5,0x6,0x7,0x8>> spec6x6patt;
	typedef utils::specialization<
			index6t<0x0,0x1,0x2,0x4,0x5,0x6>,
			index6t<0x4,0x5,0x6,0x7,0x8,0x9>,
			index6t<0x0,0x1,0x2,0x3,0x4,0x5>,
			...
			index6t<0x1,0x3,0x4,0x5,0x6,0x7>> spec7x6patt;
	typedef utils::specialization<
			index6t<0x0,0x1,0x2,0x4,0x5,0x6>,
			index6t<0x4,0x5,0x6,0x7,0x8,0x9>,
			index6t<0x0,0x1,0x2,0x3,0x4,0x5>,
			...
			index6t<0x0,0x1,0x4,0x8,0x9,0xa>> spec8x6patt;
	...
	if (opts[type]["mode"]("search") || opts[type]["mode"]("expectimax")) {
		...
		if (({ bool fixed = true; for (auto n : expectimax::depth()) fixed &= (n == (expectimax::depth()[0])); fixed; }))
			switch (to_hash(spec)) {
			case to_hash("4x6patt"): return { utils::expectimax::estimate<spec4x6patt::estimate, true>, spec4x6patt::optimize };
			case to_hash("5x6patt"): return { utils::expectimax::estimate<spec5x6patt::estimate, true>, spec5x6patt::optimize };
			case to_hash("6x6patt"): return { utils::expectimax::estimate<spec6x6patt::estimate, true>, spec6x6patt::optimize };
			case to_hash("7x6patt"): return { utils::expectimax::estimate<spec7x6patt::estimate, true>, spec7x6patt::optimize };
			case to_hash("8x6patt"): return { utils::expectimax::estimate<spec8x6patt::estimate, true>, spec8x6patt::optimize };
			default: return { utils::expectimax::estimate<utils::estimate>, utils::optimize };
			}
		else
			switch (to_hash(spec)) {
			case to_hash("4x6patt"): return { utils::expectimax::estimate<spec4x6patt::estimate, false>, spec4x6patt::optimize };
			case to_hash("5x6patt"): return { utils::expectimax::estimate<spec5x6patt::estimate, false>, spec5x6patt::optimize };
			case to_hash("6x6patt"): return { utils::expectimax::estimate<spec6x6patt::estimate, false>, spec6x6patt::optimize };
			case to_hash("7x6patt"): return { utils::expectimax::estimate<spec7x6patt::estimate, false>, spec7x6patt::optimize };
			case to_hash("8x6patt"): return { utils::expectimax::estimate<spec8x6patt::estimate, false>, spec8x6patt::optimize };
			default: return { utils::expectimax::estimate<utils::estimate>, utils::optimize };
			}
	}
	switch (to_hash(spec)) {
	case to_hash("4x6patt"): return spec4x6patt::specialized;
	case to_hash("5x6patt"): return spec5x6patt::specialized;
	case to_hash("6x6patt"): return spec6x6patt::specialized;
	case to_hash("7x6patt"): return spec7x6patt::specialized;
	case to_hash("8x6patt"): return spec8x6patt::specialized;
