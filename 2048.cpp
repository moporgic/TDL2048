...
public:
	...
	inline numeric& operator [](u64 i) { return raw[(i << 1) + i]; }
	...
	inline numeric* data(u64 i = 0) { return raw + ((i << 1) + i); }
...
private:
	inline weight(u64 sign, size_t size) : id(sign), length(size), raw(alloc(size * 3)) {}
