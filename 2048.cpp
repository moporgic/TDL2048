...
template<u32 p, u32...ps>
inline constexpr typename std::enable_if<sizeof...(ps) == 0, u64>::type mask(u64 o = 0xfull) { return (o << (p << 2)); }
template<u32 p, u32...ps>
inline constexpr typename std::enable_if<sizeof...(ps) != 0, u64>::type mask(u64 o = 0xfull) { return (o << (p << 2)) | mask<ps...>(o); }

template<u32...patt>
inline constexpr u64 indexpatt(const board& b) { return math::pext64(b, mask<patt...>()); }

//template<> u64 indexpatt<0x0,0x1,0x2,0x3>(const board& b) { return (u32(u64(b)) & 0xffff); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7>(const board& b) { return (u32(u64(b) >> 16) & 0xffff); }
//template<> u64 indexpatt<0x0,0x1,0x4,0x5>(const board& b) { return (u32(u64(b)) & 0x00ff) | (u32(u64(b) >> 8) & 0xff00); }
//template<> u64 indexpatt<0x1,0x2,0x5,0x6>(const board& b) { return (u32(u64(b) >> 4) & 0x00ff) | (u32(u64(b) >> 12) & 0xff00); }
//template<> u64 indexpatt<0x5,0x6,0x9,0xa>(const board& b) { return (u32(u64(b) >> 20) & 0x00ff) | (u32(u64(b) >> 28) & 0xff00); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x3,0x4>(const board& b) { return (u32(u64(b)) & 0xfffff); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7,0x8>(const board& b) { return (u32(u64(b) >> 16) & 0xfffff); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x4,0x5>(const board& b) { return (u32(u64(b)) & 0x00fff) | ((u32(u64(b)) >> 4) & 0xff000); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x8,0x9>(const board& b) { return (u32(u64(b) >> 16) & 0x00fff) | (u32(u64(b) >> 20) & 0xff000); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x3,0x5>(const board& b) { return (u32(u64(b)) & 0x0ffff) | (u32(u64(b) >> 4) & 0xf0000); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7,0x9>(const board& b) { return (u32(u64(b) >> 16) & 0x0ffff) | (u32(u64(b) >> 20) & 0xf0000); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>(const board& b) { return (u32(u64(b)) & 0xffffff); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>(const board& b) { return (u32(u64(b) >> 16) & 0xffffff); }
//template<> u64 indexpatt<0x8,0x9,0xa,0xb,0xc,0xd>(const board& b) { return (u32(u64(b) >> 32) & 0xffffff); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>(const board& b) { return (u32(u64(b)) & 0x000fff) | ((u32(u64(b)) >> 4) & 0xfff000); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x8,0x9,0xa>(const board& b) { return (u32(u64(b) >> 16) & 0x000fff) | (u32(u64(b) >> 20) & 0xfff000); }
//template<> u64 indexpatt<0x2,0x3,0x4,0x5,0x6,0x9>(const board& b) { return (u32(u64(b) >> 8) & 0x0fffff) | (u32(u64(b) >> 16) & 0xf00000); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x5,0x9,0xa>(const board& b) { return (u32(u64(b)) & 0x000fff) | (u32(u64(b) >> 8) & 0x00f000) | (u32(u64(b) >> 20) & 0xff0000); }
//template<> u64 indexpatt<0x3,0x4,0x5,0x6,0x7,0x8>(const board& b) { return (u32(u64(b) >> 12) & 0xffffff); }
//template<> u64 indexpatt<0x1,0x3,0x4,0x5,0x6,0x7>(const board& b) { return (u32(u64(b) >> 4) & 0x00000f) | (u32(u64(b) >> 8) & 0xfffff0); }
//template<> u64 indexpatt<0x0,0x1,0x4,0x8,0x9,0xa>(const board& b) { return (u32(u64(b)) & 0x0000ff) | (u32(u64(b) >> 8) & 0x000f00) | (u32(u64(b) >> 20) & 0xfff000); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6>(const board& b) { return (u32(u64(b)) & 0xfffffff); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7,0x8,0x9,0xa>(const board& b) { return (u32(u64(b) >> 16) & 0xfffffff); }
//template<> u64 indexpatt<0x8,0x9,0xa,0xb,0xc,0xd,0xe>(const board& b) { return (u32(u64(b) >> 32) & 0xfffffff); }
//template<> u64 indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7>(const board& b) { return (u32(u64(b))); }
//template<> u64 indexpatt<0x4,0x5,0x6,0x7,0x8,0x9,0xa,0xb>(const board& b) { return (u32(u64(b) >> 16)); }
...
struct make {
	...
	struct index4t {
		...
		template<u32 i> static typename std::enable_if<(i != 8), void>::type isomorphic(bool iso) {
			...
			make(vtos({x[t0], x[t1], x[t2], x[t3]}), index::indexpatt<x[t0], x[t1], x[t2], x[t3]>);
	...
	struct index5t {
		...
		template<u32 i> static typename std::enable_if<(i != 8), void>::type isomorphic(bool iso) {
			constexpr board x = isoindex[i];
			make(vtos({x[t0], x[t1], x[t2], x[t3], x[t4]}), index::indexpatt<x[t0], x[t1], x[t2], x[t3], x[t4]>);
			if (iso) isomorphic<i + 1>(iso);
		}
		template<u32 i> static typename std::enable_if<(i >= 8), void>::type isomorphic(bool iso) {}
	};
	...
	struct index6t {
		...
		template<u32 i> static typename std::enable_if<(i != 8), void>::type isomorphic(bool iso) {
			constexpr board x = isoindex[i];
			make(vtos({x[t0], x[t1], x[t2], x[t3], x[t4], x[t5]}), index::indexpatt<x[t0], x[t1], x[t2], x[t3], x[t4], x[t5]>);
			if (iso) isomorphic<i + 1>(iso);
		}
		template<u32 i> static typename std::enable_if<(i >= 8), void>::type isomorphic(bool iso) {}
	};
	...
	struct index7t {
		...
		template<u32 i> static typename std::enable_if<(i != 8), void>::type isomorphic(bool iso) {
			constexpr board x = isoindex[i];
			make(vtos({x[t0], x[t1], x[t2], x[t3], x[t4], x[t5], x[t6]}), index::indexpatt<x[t0], x[t1], x[t2], x[t3], x[t4], x[t5], x[t6]>);
			if (iso) isomorphic<i + 1>(iso);
		}
		template<u32 i> static typename std::enable_if<(i >= 8), void>::type isomorphic(bool iso) {}
	};
	...
	struct index8t {
		...
		template<u32 i> static typename std::enable_if<(i != 8), void>::type isomorphic(bool iso) {
			constexpr board x = isoindex[i];
			make(vtos({x[t0], x[t1], x[t2], x[t3], x[t4], x[t5], x[t6], x[t7]}), index::indexpatt<x[t0], x[t1], x[t2], x[t3], x[t4], x[t5], x[t6], x[t7]>);
			if (iso) isomorphic<i + 1>(iso);
		}
		template<u32 i> static typename std::enable_if<(i >= 8), void>::type isomorphic(bool iso) {}
	};
...
struct method {
	...
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x8,0x9,0xa>> iso4x6patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>,
			index::indexpatt<0x8,0x9,0xa,0xb,0xc,0xd>,
			index::indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x8,0x9,0xa>> iso5x6patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>,
			index::indexpatt<0x2,0x3,0x4,0x5,0x6,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x5,0x9,0xa>,
			index::indexpatt<0x3,0x4,0x5,0x6,0x7,0x8>> iso6x6patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>,
			index::indexpatt<0x2,0x3,0x4,0x5,0x6,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x5,0x9,0xa>,
			index::indexpatt<0x3,0x4,0x5,0x6,0x7,0x8>,
			index::indexpatt<0x1,0x3,0x4,0x5,0x6,0x7>> iso7x6patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9>,
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5>,
			...
			index::indexpatt<0x0,0x1,0x2,0x5,0x9,0xa>,
			index::indexpatt<0x3,0x4,0x5,0x6,0x7,0x8>,
			index::indexpatt<0x1,0x3,0x4,0x5,0x6,0x7>,
			index::indexpatt<0x0,0x1,0x4,0x8,0x9,0xa>> iso8x6patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9,0xa>> iso2x7patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9,0xa>,
			index::indexpatt<0x8,0x9,0xa,0xb,0xc,0xd,0xe>> iso3x7patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7>> iso1x8patt;
	typedef isomorphic::static_index<
			index::indexpatt<0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7>,
			index::indexpatt<0x4,0x5,0x6,0x7,0x8,0x9,0xa,0xb>> iso2x8patt;
