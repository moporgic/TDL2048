...
inline numeric estimate_4x6patt(const board& state,
	...
	esti += f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)];
	esti += f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)];
	esti += f[2 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)];
	esti += f[3 << 3][index6t<0x4,0x5,0x6,0x8,0x9,0xa>(iso)];
	iso.mirror();
	esti += f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)];
	esti += f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)];
	esti += f[2 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)];
...
inline numeric optimize_4x6patt(const board& state, numeric error, numeric alpha,
	...
	register feature* f = range.begin();
	register board iso = state;
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)] += updv);
	esti += (f[3 << 3][index6t<0x4,0x5,0x6,0x8,0x9,0xa>(iso)] += updv);
	iso.mirror();
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)] += updv);
...
inline numeric estimate_5x6patt(const board& state,
		clip<feature> range = feature::feats()) {
	register numeric esti = 0;
	register feature* f = range.begin();
	register board iso = state;
	esti += f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)];
	esti += f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)];
	esti += f[2 << 3][index6t<0x8,0x9,0xa,0xb,0xc,0xd>(iso)];
	esti += f[3 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)];
...
inline numeric optimize_5x6patt(const board& state, numeric error, numeric alpha,
	...
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x8,0x9,0xa,0xb,0xc,0xd>(iso)] += updv);
	esti += (f[3 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)] += updv);
	esti += (f[4 << 3][index6t<0x4,0x5,0x6,0x8,0x9,0xa>(iso)] += updv);
	iso.mirror();
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x8,0x9,0xa,0xb,0xc,0xd>(iso)] += updv);
...
inline numeric estimate_8x6patt(const board& state,
		clip<feature> range = feature::feats()) {
	register numeric esti = 0;
	register feature* f = range.begin();
	register board iso = state;
	esti += f[0 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)];
	esti += f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)];
	esti += f[2 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)];
	esti += f[3 << 3][index6t<0x2,0x3,0x4,0x5,0x6,0x9>(iso)];
	esti += f[4 << 3][index6t<0x0,0x1,0x2,0x5,0x9,0xa>(iso)];
	esti += f[5 << 3][index6t<0x3,0x4,0x5,0x6,0x7,0x8>(iso)];
	esti += f[6 << 3][index6t<0x1,0x3,0x4,0x5,0x6,0x7>(iso)];
	esti += f[7 << 3][index6t<0x0,0x1,0x4,0x8,0x9,0xa>(iso)];
	iso.mirror();
	esti += f[0 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)];
	esti += f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)];
	esti += f[2 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)];
...
inline numeric optimize_8x6patt(const board& state, numeric error, numeric alpha,
	...
	register feature* f = range.begin();
	register board iso = state;
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
	esti += (f[3 << 3][index6t<0x2,0x3,0x4,0x5,0x6,0x9>(iso)] += updv);
	esti += (f[4 << 3][index6t<0x0,0x1,0x2,0x5,0x9,0xa>(iso)] += updv);
	esti += (f[5 << 3][index6t<0x3,0x4,0x5,0x6,0x7,0x8>(iso)] += updv);
	esti += (f[6 << 3][index6t<0x1,0x3,0x4,0x5,0x6,0x7>(iso)] += updv);
	esti += (f[7 << 3][index6t<0x0,0x1,0x4,0x8,0x9,0xa>(iso)] += updv);
	iso.mirror();
	esti += (f[0 << 3][index6t<0x0,0x1,0x2,0x4,0x5,0x6>(iso)] += updv);
	esti += (f[1 << 3][index6t<0x4,0x5,0x6,0x7,0x8,0x9>(iso)] += updv);
	esti += (f[2 << 3][index6t<0x0,0x1,0x2,0x3,0x4,0x5>(iso)] += updv);
...
statistic train(utils::options opts = {}) {
	...
	case to_hash("forward-best-5x6patt"):
		for (stats.init(opts["train"]); stats; stats++) {
			...
			b.init();
			best(b, feats, utils::estimate_5x6patt);
			score += best.score();
			opers += 1;
			best >> last;
			best >> b;
			b.next();
			while (best(b, feats, utils::estimate_5x6patt)) {
				last.optimize(best.esti(), alpha, feats, utils::optimize_5x6patt);
				score += best.score();
				opers += 1;
				best >> last;
				best >> b;
			...
			last.optimize(0, alpha, feats, utils::optimize_5x6patt);
...
statistic test(utils::options opts = {}) {
	...
	case to_hash("best-5x6patt"):
		for (stats.init(opts["test"]); stats; stats++) {
			...
			for (b.init(); best(b, feats, utils::estimate_5x6patt); b.next()) {
				score += best.score();
				opers += 1;
				best >> b;
			}
